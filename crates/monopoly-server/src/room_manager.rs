...
use crate::actor::{spawn_actor, ActorMsg, JoinResult, PlayerIdentity, Session};
...
impl RoomHandle {
    ...
    /// 请求入座（走 Actor，保证串行加入）。
    ///
    /// 携带全局身份时：同 uid 已有座位则认领原座位（跨天/重进恢复原角色）。
    pub async fn request_join(&self, identity: PlayerIdentity) -> Result<JoinResult, ApiError> {
        let (tx, rx) = oneshot::channel();
        self.sender
            .send(ActorMsg::Join { identity, reply: tx })
            .await
